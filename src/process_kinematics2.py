...
from baxter_force_control.steppables import CostCalculator, EdgePublisher, BagReader, JointStateMsgReader, \
    TwistMsgReader, TFMsgFrameReader, JacobianCalculator, TwistTransformer, Transformer, Vector3MsgReader, \
    CostGradient, WeightedCostGradients, Merger, Magnitude, Divider, WeightedKinematicCostDescentEstimator
...
def process(trial_bag, box_kin_tree, human_kin_tree, dt, config_cost, ergo_cost, diagram_filename=None, bag=None, pos_only=True):
    ...
    H_J_HG_node = ForwardBlockNode(arm_jac, 'Chest to Grip Arm Jacobian', 'C_G_C_G_twist')
    ...
    estimator = WeightedKinematicCostDescentEstimator()
    relative_weight_node = ForwardBlockNode(estimator, 'Relative Weight Estimator', 'weights_relative')
    estimator_nnls = WeightedKinematicCostDescentEstimator(disturbance=True, method='NNLS')
    relative_weight_nnls_node = ForwardBlockNode(estimator_nnls, 'Relative Weight Estimator NNLS', 'weights_relative_nnls')
    
    estimator_static = WeightedKinematicCostDescentEstimator(disturbance=False)
    static_weight_node = ForwardBlockNode(estimator_static, 'Static Weight Estimator', 'weights_static')
    estimator_static_nnls = WeightedKinematicCostDescentEstimator(disturbance=False, method='NNLS')
    static_weight_nnls_node = ForwardBlockNode(estimator_static_nnls, 'Static Weight Estimator NNLS', 'weights_static_nnls')
    
    dCdx_node.add_output(relative_weight_node, 'cost_descents')
    dCdx_node.add_output(static_weight_node, 'cost_descents')
    
    state_deriv_node.add_output(relative_weight_node, 'input')
    state_deriv_node.add_output(static_weight_node, 'input')
    
    J_RG_node.add_output(relative_weight_node, 'J_RG')
    J_RG_node.add_output(static_weight_node, 'J_RG')
    J_HG_node.add_output(relative_weight_node, 'J_HG')
    J_HG_node.add_output(static_weight_node, 'J_HG')

    R_G_v_R_H_node.add_output(relative_weight_node, 'RG_V_RH')
    R_G_v_R_H_node.add_output(static_weight_node, 'RG_V_RH')
    
    dCdx_node.add_output(relative_weight_nnls_node, 'cost_descents')
    dCdx_node.add_output(static_weight_nnls_node, 'cost_descents')
    
    state_deriv_node.add_output(relative_weight_nnls_node, 'input')
    state_deriv_node.add_output(static_weight_nnls_node, 'input')
    
    J_RG_node.add_output(relative_weight_nnls_node, 'J_RG')
    J_RG_node.add_output(static_weight_nnls_node, 'J_RG')
    J_HG_node.add_output(relative_weight_nnls_node, 'J_HG')
    J_HG_node.add_output(static_weight_nnls_node, 'J_HG')

    R_G_v_R_H_node.add_output(relative_weight_nnls_node, 'RG_V_RH')
    R_G_v_R_H_node.add_output(static_weight_nnls_node, 'RG_V_RH')

    merger = Merger(['_relative', '_static', '_relative_nnls', '_static_nnls'] + ['']*8)
    merger_node = ForwardBlockNode(merger, 'Output Merger', 'output')
    relative_weight_node.add_output(merger_node, 'first')
    static_weight_node.add_output(merger_node, 'second')
    relative_weight_nnls_node.add_output(merger_node, 'third')
    static_weight_nnls_node.add_output(merger_node, 'fourth')
    ...
    ergo_cost_node.add_output(merger_node, 'twelfth')
