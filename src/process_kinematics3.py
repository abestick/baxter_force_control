...
from baxter_force_control.steppables import CostCalculator, EdgePublisher, BagReader, JointStateMsgReader, \
    ...
    CostGradient, WeightedCostGradients, Merger, Magnitude, WeightedKinematicCostDescentEstimatorBases, JacobianOperator
...
def process(trial_bag, box_kin_tree, human_kin_tree, dt, config_cost, ergo_cost, diagram_filename=None, bag=None, pos_only=True):
    ...
    relative_weight_node.add_output(relative_node, 'bases')
    relative_weight_node.add_output(nnls_relative_node, 'bases')

    static_weight_node.add_output(static_node, 'bases')
    static_weight_node.add_output(nnls_static_node, 'bases')
